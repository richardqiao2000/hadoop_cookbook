...
describe 'hadoop::_sql_connectors' do
  context 'Using MySQL on CentOS 6.9 x86_64' do
    let(:chef_run) do
  ...
  context 'Using MySQL on Ubuntu 14.04 on CDH' do
    let(:chef_run) do
  ...
  context 'using PostgreSQL on CentOS 6.9 x86_64' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new(platform: 'centos', version: 6.9) do |node|
  ...
  context 'Using PostgreSQL on Ubuntu 14.04' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new(platform: 'ubuntu', version: 14.04) do |node|
        node.default['hadoop']['sql_connector'] = 'postgresql'
      end.converge(described_recipe)
